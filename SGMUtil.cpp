...
void SGMUtil::costAggregateLeftRight(const uint8 *imgData, const sint32 &width, const sint32 &height,
                                     ...
                                     const sint32 &p2Init, const float32 *costInit, float32 *costAggr, bool isForward) {
    ...
    for (sint32 i = 0; i < height; i++) {
        ...
        std::vector<float32> costLastPath(dispRange + 2, UINT8_MAX);
        // 初始化：第一个像素的聚合代价值等于初始代价值
        memcpy(costAggrRow, costInitRow, dispRange * sizeof(float32));
        memcpy(&costLastPath[1], costAggrRow, dispRange * sizeof(float32));
        costInitRow += direction * dispRange;
        costAggrRow += direction * dispRange;
        imgRow += direction;

        // 路径上上个像素的最小代价值
        float32 minCostLastPath = UINT8_MAX;
        ...
        for (sint32 j = 0; j < width - 1; j++) {
            gray = *imgRow;
            float32 minCost = UINT8_MAX;
            for (sint32 d = 0; d < dispRange; d++) {
                // Lr(p,d) = C(p,d) + min( Lr(p-r,d), Lr(p-r,d-1) + P1, Lr(p-r,d+1) + P1, min(Lr(p-r))+P2 ) - min(Lr(p-r))
                const float32 cost = costInitRow[d];

                const float32 l1 = costLastPath[d + 1];
                const float32 l2 = costLastPath[d] + P1;
                const float32 l3 = costLastPath[d + 2] + P1;
                const float32 l4 = minCostLastPath + std::max(P1, P2Init / (abs(gray - grayLast) + 1));
                float32 costNew =
                        cost + static_cast<float32>(std::min(std::min(l1, l2), std::min(l3, l4)) - minCostLastPath);
...
void SGMUtil::costAggregateUpDown(const uint8 *imgData, const sint32 &width, const sint32 &height,
    ...
    for (sint32 j = 0; j < width; j++) {
        ...
        // 初始化：第一个像素的聚合代价值等于初始代价值
        memcpy(costAggrCol, costInitCol, dispRange * sizeof(float32));
        memcpy(&costLastPath[1], costAggrCol, dispRange * sizeof(float32));
        ...
        for (sint32 i = 0; i < height - 1; i++) {
            ...
            for (sint32 d = 0; d < dispRange; d++) {
                // Lr(p,d) = C(p,d) + min( Lr(p-r,d), Lr(p-r,d-1) + P1, Lr(p-r,d+1) + P1, min(Lr(p-r))+P2 ) - min(Lr(p-r))
                const float32 cost = costInitCol[d];
...
void SGMUtil::costAggregateDiagonalLeftRight(const uint8 *imgData, const sint32 &width, const sint32 &height,
                                             const sint32 &minDisparity, const sint32 &maxDisparity, const sint32 &p1,
                                             const sint32 &p2Init, const float32 *costInit, float32 *costAggr,
                                             bool isForward) {
